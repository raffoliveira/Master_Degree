...
def filter_data(data, filter, freq):
    bpFilt = firwin(5, [0.5, 40], pass_zero=False, fs=freq)
...
def outlier_removal(data_paths, dataFilt_paths, qrs_dict, ids_rhythm, rhythm_map,  sizeBeat, path):
    ...
    ids = len(data_paths)
    ann_valid_dict = {}
    ii = 1 #SELECIONA QUAL DERIVAÇÃO USAR. 0-11
    ...
    for id in range(ids):
        *rest, id_file = data_paths[id].split('/')
        ...
        for jj in range(len(anns[ii])):
            ...
            beatWaveDII = ecgDII[ii, startPoint:endPoint].reshape(1, -1)
            beatWaveDII_t = ecgDII_t[ii, startPoint:endPoint].reshape(1, -1)
    ...
    for id in range(ids):
        *rest, id_file = data_paths[id].split('/')
        ...
        if rhythm == 'SR' and samples>0:
            ...
            m1_t = np.mean(train_ecg_t, axis=0)
            m1_t = m1_t[np.isfinite(m1_t)]
            m1_mean_t = np.mean(m1_t)
            data = data - m1_mean_t

            m1 = np.mean(train_ecg, axis=0)
    ...
    for id in range(ids):
        *rest, id_file = data_paths[id].split('/')
        ...
        if rhythm != 'SR' and samples > 0:
            m2_t = np.mean(train_ecg_t, axis=0)
            ...
            for jj in range(len(anns[ii])):
                ...
                if beatWaveDII.shape[0] == sizeBeat and ann_valid[jj] == 1:
                    ...
                    if useDataAugmentation:
                        ...
                        # atenua a onda P
                        sample['data'] = beatWaveDII * np.concatenate((np.full((60,1), 0.3), np.ones((340,1))), axis=0)
                        sample['label'] = label_id
                        sample['thbc'] = thbc
                        sample['set'] = set
                        sample['da'] = 1
                        ...
                        # da ganho a onda P
                        sample['data'] = beatWaveDII * np.concatenate((np.full((60,1), 1.2), np.ones((340,1))), axis=0)
                        sample['label'] = label_id
                        sample['thbc'] = thbc
                        sample['set'] = set
                        sample['da'] = 1
                        ...
                        # atenua a onda T
                        sample['data'] = beatWaveDII * np.concatenate((np.ones((150,1)), np.full((250,1), 0.3)), axis=0)
                        sample['label'] = label_id
                        sample['thbc'] = thbc
                        sample['set'] = set
                        sample['da'] = 1
                        ...
                        # da ganho a onda T
                        sample['data'] = beatWaveDII * np.concatenate((np.ones((150,1)), np.full((250,1), 1.2)), axis=0)
                        sample['label'] = label_id
                        sample['thbc'] = thbc
                        sample['set'] = set
                        sample['da'] = 1
...
#                classes[rhythm] = classes[rhythm] + 1
            ...
            for jj in range(len(anns[ii])):
                ...
                if beatWaveDII.shape[0] and ann_valid[jj] == 1 and limiter < 5:
                    if aux == 0:
           #             label_id = label_id + 1
                        aux = 1
