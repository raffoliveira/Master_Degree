...
def outlier_removal(data_paths, dataFilt_paths, qrs_dict, ids_rhythm, rhythm_map,  sizeBeat, path):
    ...
    for id in range(ids):
        *rest, id_file = data_paths[id].split('/')
        ...
        for ii in range(len(anns)):
            train_ecg[ii] = []
            train_ecg_t[ii] = []
            valid_aux = np.zeros(len(anns[ii]))
            ...
            for jj in range(len(anns[ii])):
                ...
                beatWaveDII = ecgDII[ii][startPoint:endPoint]
                beatWaveDII_t = ecgDII_t[ii][startPoint:endPoint]

                if len(beatWaveDII) == sizeBeat:
                    valid = 1
                    #criterio 1: se valor maximo nao for o QRS, exclui
                    max_pos = np.argmax(beatWaveDII)
                    ...
                    if valid == 1:
                        train_ecg[ii].append(beatWaveDII)
                        train_ecg_t[ii].append(beatWaveDII_t)

            ann_valid.append(valid_aux)

        train_ecg = [train_ecg[i] for i in train_ecg.keys()]
        train_ecg_t = [train_ecg_t[i] for i in train_ecg_t.keys()]
    ...
    print('\tScale the beatles according to Sinus Rhythm (normal rhythm) ...')
    ...
    for id in range(ids):
        *rest, id_file = data_paths[id].split('/')
        ...
        if rhythm == 'SR':
            ...
            m1_t = np.array([np.mean(train_ecg_t[i]) for i in range(12)])
            m1_t = m1_t[np.isfinite(m1_t)]
            m1_mean_t = np.mean(m1_t)
            data = data - m1_mean_t

            m1 = np.array([np.mean(train_ecg[i]) for i in range(12)])
    ...
    for id in range(ids):
        *rest, id_file = data_paths[id].split('/')
        ...
        if rhythm != 'SR' and samples > 0:
            m2_t = np.array([np.mean(train_ecg_t[i]) for i in range(12)])
            ...
            m2 = np.array([np.mean(train_ecg[i]) for i in range(12)])
    ...
    for id in range(ids):
        ...
        *rest, id_file = data_paths[id].split('/')
        ...
        ecg = []
        for i in range(len(train_ecg)):
            for j in range(len(train_ecg[i])):
                ecg.append(train_ecg[i][j])
        ecg = np.asarray(ecg)

        mean_ecg = np.mean(ecg, axis=0).reshape((1,-1))
        ...
        for ii in range(len(anns)):
            ...
            for jj in range(len(anns[ii])):
                ...
                if len(beatWaveDII) == sizeBeat:
                    ...
                    if valid == 1:
                        beatWaveDII = beatWaveDII.reshape((1, -1))
        ...
        for ii in range(len(anns)):
            ...
            for jj in range(len(anns[ii])):
                ...
                if len(beatWaveDII) == sizeBeat:
                    ...
                    if valid_aux[jj] == 1:
                        d = float(np.abs(euclidean_distances(beatWaveDII, mean_ecg)))
    ...
    for id in range(ids):
        *rest, id_file = data_paths[id].split('/')
        ...
        total = [len(i) for i in ann]
        total = np.sum(total)

        ann_valid = ann_valid_dict[id_file]
        valids = [np.sum(i) for i in ann_valid]
        ...
        if rhythm == 'SR':
            ...
            for ii in range(len(anns)):
                for jj in range(len(anns[ii])):
                    ...
                    if len(beatWaveDII) == sizeBeat and ann_valid[ii][jj] == 1:
                        if aux == 0:
                            label_id = label_id + 1
                            aux = 1
                        ...
                        if useDataAugmentation:
                            ...
                            # atenua a onda P
                            sample['data'] = beatWaveDII * np.concatenate((np.full((60), 0.3), np.ones((340))), axis=None)
                            sample['label'] = label_id
                            sample['thbc'] = thbc
                            sample['set'] = set
                            sample['da'] = 1
                            ...
                            # da ganho a onda P
                            sample['data'] = beatWaveDII * np.concatenate((np.full((60), 1.2), np.ones(340)), axis=None)
                            sample['label'] = label_id
                            sample['thbc'] = thbc
                            sample['set'] = set
                            sample['da'] = 1
                            ...
                            # atenua a onda T
                            sample['data'] = beatWaveDII * np.concatenate((np.ones((150)), np.full((250), 0.3)), axis=None)
                            sample['label'] = label_id
                            sample['thbc'] = thbc
                            sample['set'] = set
                            sample['da'] = 1
                            ...
                            # da ganho a onda T
                            sample['data'] = beatWaveDII * np.concatenate((np.ones((150)), np.full((250), 1.2)), axis=None)
                            sample['label'] = label_id
                            sample['thbc'] = thbc
                            sample['set'] = set
                            sample['da'] = 1
...
#                classes[rhythm] = classes[rhythm] + 1
            ...
            for ii in range(len(anns)):
                ...
                for jj in range(len(anns[ii])):
                    ...
                    #CONFERIR SINGLE FLAG

                    if len(beatWaveDII) == sizeBeat and ann_valid[ii][jj] == 1 and limiter<2:
                        if aux == 0:
               #             label_id = label_id + 1
                            aux = 1
